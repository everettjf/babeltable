...
- Short utterances have no four-character threshold. Both candidate outputs are retained for routing, including output received before input. Recognized language changes split rapid exchanges. Reconnecting closes the previous connection's pending turns.
...
- Final source verification: 56 XCTest + 1 Swift Testing tests passed on the iPhone 17 simulator; final Release simulator build passed. Result bundle: `/tmp/babeltable-closeout-delivery.xcresult` on the verification host.
